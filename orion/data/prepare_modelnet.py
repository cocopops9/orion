...
def binlist_to_hdf5(bins_list, h5_filename, h5_text_filename, chunk_size=1000):
    ...
    # Create text file listing all chunks (relative names only)
    with open(h5_text_filename, 'w') as f:
        for c in range(1, nchunks + 1):
            base = os.path.basename(h5_filename)
            chunk_name = base if c == 1 else f"{base}.{c:04d}"
            f.write(chunk_name + '\n')
    ...
    for c in range(1, nchunks + 1):
        ...
        # Load binary voxel files — store in PyTorch convention (N, D, H, W)
        data = np.zeros((this_chunk_size, d1, d2, d3), dtype=np.uint8)
        ...
        with h5py.File(chunk_name, 'w') as hf:
            ...
            hf.create_dataset('label', data=np.array(labels_class[ind1:ind2], dtype=np.int64))
            hf.create_dataset('label_pose', data=np.array(labels_pose[ind1:ind2], dtype=np.int64))
